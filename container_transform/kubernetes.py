...
class KubernetesTransformer(BaseTransformer):
    ...
    def emit_containers(self, containers, verbose=True):
        ...
        containers_dict = json.loads(json.dumps(containers))
        #print("***********************")
        ...
        output = {
                    "jobs": [
                        {
                            "name": containers_dict[0].get("name"),
                            "securityContext": {
                                "runAsUser": 1000,
                                "runAsGroup": 1000,
                                "fsGroup": 2000
                            ...
                            "image": {
                                "repository":  containers_dict[0].get("image", None),
                                "tag": "latest",
                                "imagePullPolicy": "Always"
                            },
                            "env": containers_dict[0].get("env"),
                            "envFrom": [
                                {
                                    "secretRef": None,
                                    "name": "SECRET_DATA"
                            ...
                            "schedule": containers_dict[0].get("schedules").get("cron"),
                            "command": [
                                containers_dict[0].get("command", None),
                            ],
                            "args": [
                                "-c",
                            ...
                            "concurrencyPolicy": str(containers_dict[0].get("schedules").get("concurrencyPolicy")).title(),
                            "restartPolicy": str(containers_dict[0].get("restartpolicy")).title(),
                            "nodeSelector": {
                                "type": "infra"
                            },
                            "tolerations": [
    ...
    def ingest_max_launch_delay(self, max_launch_delay):
        return max_launch_delay

    def emit_max_launch_delay(self, max_launch_delay):
        raise NotImplementedError

    def ingest_schedules(self, schedules):
       raise NotImplementedError

    def emit_schedules(self, schedules):
        return schedules

    def ingest_restart_policy(self, restart_policy):
        return restart_policy

    def emit_restart_policy(self, restart_policy):
        return restart_policy
