...
from .schema import TransformationTypes, ARG_MAP, ARG_MAP_JOB
...
class MarathonTransformer(BaseTransformer):
    ...
    * A JSON array of Marathon application objects
    ...
    def flatten_container(self, container):
        ...
        """
        if "run" in container:
            arg_map_values = ARG_MAP_JOB.values()
        else:
            arg_map_values = ARG_MAP.values()
        for names in arg_map_values:
    ...
    def ingest_environment(self, environment=None):
        input_data = self.stream
        if isinstance(input_data, dict):
            environment = input_data.get("run").get("env")
        return environment
    ...
    def ingest_schedules(self, schedules):
        data = {
            "cron": [schedule.get("cron") for schedule in schedules][0],
            "concurrencyPolicy" : [schedule.get("concurrencyPolicy") for schedule in schedules][0]
        }
        return data
